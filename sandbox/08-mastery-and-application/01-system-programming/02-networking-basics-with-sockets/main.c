...
void udp_server(int port) {
    ...
    while (keep_running) {
        // Receive datagram
        memset(buffer, 0, sizeof(buffer));
        int bytes_read = recvfrom(server_fd, buffer, sizeof(buffer) - 1, 0,
...
void udp_client(const char *server_ip, int port) {
    ...
    while (keep_running) {
        ...
        ssize_t bytes_sent = sendto(sock_fd, buffer, strlen(buffer), 0,
        ...
        // Receive response
        memset(buffer, 0, sizeof(buffer));
        int bytes_read = recvfrom(sock_fd, buffer, sizeof(buffer) - 1, 0,
                                  (struct sockaddr*)&server_addr, &server_len);
