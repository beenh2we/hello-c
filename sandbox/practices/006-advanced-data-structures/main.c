...
float calculate_triangle_area(const Shape *shape)
{
    ...
    float a = sqrtf((p[1].x - p[0].x) * (p[1].x - p[0].x) + (p[1].y - p[0].y) * (p[1].y - p[0].y));
    float b = sqrtf((p[2].x - p[1].x) * (p[2].x - p[1].x) + (p[2].y - p[1].y) * (p[2].y - p[1].y));
    float c = sqrtf((p[0].x - p[2].x) * (p[0].x - p[2].x) + (p[0].y - p[2].y) * (p[0].y - p[2].y));
...
void render_circle(const Shape *shape)
{
    ...
    printf(
        "  Color: RGBA(%u, %u, %u, %u)\n", shape->color.red, shape->color.green, shape->color.blue, shape->color.alpha);
}
...
void render_triangle(const Shape *shape)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        printf("  Point %d: (%.1f, %.1f)\n", i + 1, shape->data.triangle.points[i].x, shape->data.triangle.points[i].y);
    }
    printf(
        "  Color: RGBA(%u, %u, %u, %u)\n", shape->color.red, shape->color.green, shape->color.blue, shape->color.alpha);
}
...
int main()
{
    ...
    // Create a triangle
    Point triangle_points[3] = {{0.0f, 0.0f}, {5.0f, 10.0f}, {10.0f, 0.0f}};
    ...
    AreaCalculator area_calculators[3] = {calculate_circle_area, calculate_rectangle_area, calculate_triangle_area};
