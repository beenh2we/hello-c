...
void error_logger_stderr(const ErrorState *error) {
    ...
    switch (error->level) {
        case ERROR_WARNING: level_str = "WARNING"; break;
        case ERROR_ERROR:   level_str = "ERROR"; break;
        case ERROR_FATAL:   level_str = "FATAL"; break;
...
FILE* safe_fopen(const char *filename, const char *mode) {
    ...
    if (file == NULL) {
        SET_ERROR(ERROR_ERROR, errno, "Failed to open file '%s' with mode '%s'", 
                 filename, mode);
    }
    return file;
}
