...
void* hardware_simulation(void* arg) {
    while (!stop_simulation) {
        ...
        if (device->CONTROL & 0x01) { // If device is enabled
            device->DATA = (device->DATA + 1) % 100; // Increment data
        }
        
        // Simulate interrupt generation
        if ((device->CONTROL & 0x02) && (rand() % 10 == 0)) { // 10% chance of interrupt if enabled
            device->INTERRUPT = 1;
        }
...
void demonstrate_optimization_issue() {
    ...
    while (non_volatile_status != 0 && counter < 5) {
        ...
        non_volatile_status = device->STATUS & 0xFF; // This actually changes it, but compiler might not realize
        counter++;
    }
