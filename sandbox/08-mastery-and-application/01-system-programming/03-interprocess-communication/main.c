...
void message_queue_demo() {
    ...
    if (pid == 0) {
        ...
        snprintf(msg.msg_text, sizeof(msg.msg_text), 
                 "Hello from child process via message queue (PID: %d)", getpid());
...
void shared_memory_demo() {
    ...
    if (pid == 0) {
        ...
        snprintf(shared_data->message, sizeof(shared_data->message),
                 "Hello from child process via shared memory (PID: %d)", getpid());
...
void posix_shm_demo() {
    ...
    if (pid == 0) {
        ...
        snprintf(shared_data->message, sizeof(shared_data->message),
                 "Hello from child via POSIX shared memory (PID: %d)", getpid());
        shared_data->timestamp = time(NULL);
